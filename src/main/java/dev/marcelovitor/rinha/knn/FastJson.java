...
final class FastJson {
    ...
    static int findKey(byte[] b, int len, byte[] key, int from) {
        outer:
        for (int i = Math.max(0, from); i <= len - key.length; i++) {
            if (b[i] != key[0]) continue;
            for (int j = 1; j < key.length; j++) {
                if (b[i + j] != key[j]) continue outer;
            }
            return i;
        }
        return -1;
    ...
    static double numberAfterKey(byte[] b, int len, byte[] key, int from) {
        int p = positionAfterColon(b, len, key, from);
    ...
    static boolean boolAfterKey(byte[] b, int len, byte[] key, int from) {
        int p = positionAfterColon(b, len, key, from);
        if (p < len && b[p] == 't') return true;
        if (p < len && b[p] == 'f') return false;
    ...
    static boolean stringInArray(byte[] b, int len, byte[] key, int from, long needle) {
        ...
        for (int i = p; i < end; i++) {
            if (b[i] == '"') {
                ...
                int e = s;
                while (e < end && b[e] != '"') e++;
                if (rangeEquals(b, s, e, needleStart, needleEnd)) return true;
                i = e;
            }
        }
        return false;
    ...
    private static int positionAfterColon(byte[] b, int len, byte[] key, int from) {
        int p = findKey(b, len, key, from);
        if (p < 0) throw new IllegalArgumentException("missing key");
        p += key.length;
        while (p < len && b[p] != ':') p++;
        if (p >= len) throw new IllegalArgumentException("missing value");
        p++;
        while (p < len && isSpace(b[p])) p++;
        return p;
    }

    private static boolean rangeEquals(byte[] b, int s1, int e1, int s2, int e2) {
        int n = e1 - s1;
