...
public final class IndexBinReader {
    private IndexBinReader() {}

    public static FraudDetector readFrom(Path path) throws IOException {
        String s = path.toString();
        if (s.endsWith(".bin")) return IvfIndex.load(path);
        return ReferenceCatalog.load(path);
    }
